...
class ConferenceplusModelPayments extends ConferenceplusModelDefault
{
	...
	{
		// Load the filters.
		$this->setState('filter.event_id', $this->getUserStateFromRequest('filter.payments.event_id', 'eventname', ''));
	}
	...
	public function buildQuery($overrideLimits = false)
	{
		...
		{
			// Join ticket
			$query->join('INNER', '#__conferenceplus_tickets AS t ON pay.conferenceplus_payment_id = t.payment_id');

			// Join tickettype
			$query->join('INNER', '#__conferenceplus_tickettypes AS tt ON tt.conferenceplus_tickettype_id = t.tickettype_id');
